...
class Clients extends MY_Controller 
{
     ...
     public function addppt($cid,$pid)
    {
        ...
        if(!empty($_FILES['ppt']['name']) && count(array_filter($_FILES['ppt']['name'])) > 0){ 
      ...
      //if(!empty($_FILES['ppt']['name'])){
       ...
       /* $cnt = $_FILES['ppt']['name'];
            for($x=0;$x<count($cnt);$x++){
                $post=$this->input->post();
                //$data =  $this->qm->upload('./external/uploads/','ppt',$exe);
                $data = $this->multiupload('ppt',$cid,$pid,$post);
                $post['ppt']=$data;
                $post['cid']=$cid;
                $post['pid']=$pid;
                $pp = $this->qm->insert("upload_ppt_ri",$post);
        ...
        if(!empty($_FILES['card']['name'])){
           ...
           if($ins){
            ...
            redirect('clients/employees/'.$cid.'/'.$pid.'');
             ...
             $this->session->set_flashdata('eroor', 'Some Error Occurred');
       ...
       if(isset($_FILES['efile']['name']) && in_array($_FILES['efile']['type'], $file_mimes)) {
        ...
        if (!empty($sheetData)) {
            ...
            for ($i=0; $i<count($sheetData); $i++) {
                ...
                $dataa = array(
                    'dol' => (!empty($dol)) ? date("Y-m-d", strtotime($dol)) : date('Y-m-d'),
                    'reson' => $reason,
                    'mode' => $mode,
                    'status' => $status,
                );
                ...
                if($relation == "Self"){
                    ...
                    if(count($check) > 0){
                        $where = array('pid' => $pid,'cid' => $cid,'emp_id'=>$emp_id);
                        $this->qm->update('ri_employee_tbl',$dataa,$where);
                        $this->qm->update('ri_dependent_tbl',$dataa,$where);
                    }
                } else {
                    if($relation != "Kid") {
