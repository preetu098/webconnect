		...
		<div class="">
			...
			<div class="row kanban-bx">
				...
				foreach ($result as $k => $row) {
					...
					<div class="col">
						<div class="kanbanPreview-bx">
							<div class="draggable-zone invisible-checkboxes  dropzoneContainer">
								<input type="radio" name="rGroup" <?= ($row->id == $this->uri->segment(3)) ? "checked" : "" ?>
									class="checkboxnew" value="<?= $myid ?>" id="r<?= $myid ?>" />
								<label class="checkbox-alias" for="r<?= $myid ?>">
									<div class="card draggable-handle draggable"
										style="background: #f8f8f8 !important; border: 0 !important;">
										<div class="card-body" style="padding: 0.875rem; margin: -13px;">
											...
											<h6>Corporate Neame</h6>
														...
														<div class="col-md-4">
															...
															<div class="brand-logo">
																<center><img
																		src="<?= base_url('external/uploads/' . $row->iimage) ?>"
														...
														<div class="plcy-card plcy-card-bottom">
															...
															<div class="row">
																<div class="col-md-3">
																	<h4>Valid From</h4>
																	<h6>
																		<?= date_format(date_create($row->sdate), "d-m-Y"); ?>
																	</h6>
																</div>
																<div class="col-md-3">
																	<h4>Valid Till</h4>
																	<h6>
																		<?= date_format(date_create($row->edate), "d-m-Y") ?>
																	</h6>
																</div>
																<div class="col-md-3">
																	<h4>Sum Insured (s)</h4>
													...
													<div class="col-md-12">
														<div class="card-btn-section" style="background:#136eca">
															<div class="card-btn-left">
																<a href="<?= base_url('client/focusedclaim/' . $row->id) ?>"
																	class="btn btn-outline-primary btn-rounded fs-18 card-slider-btn">Claims
																	Status</a>
																<a href="<?= base_url('client/endorsement/' . $row->id) ?>"
																	class="btn btn-outline-primary btn-rounded fs-18 card-slider-btn">Endorsements</a>
																<a href="<?= $link ?>"
																	class="btn btn-outline-primary btn-rounded fs-18 card-slider-btn"
																	download>Download Policy</a>
																<a data-bs-toggle="modal" data-bs-target="#pop<?= $k + 1 ?>"
																	class="btn btn-outline-primary btn-rounded fs-18 card-slider-btn">View
																	PPT </a>
															</div>
														</div>
													</div>
					...
					<div class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" id="pop<?= $k + 1 ?>"
						aria-hidden="true">
								...
								<div class="modal-body">
									<div class="col-md-12 col-sm-12">
										<div class="ppt-slide-bx">
											<div id="carouselExampleIndicators<?= $k + 1 ?>" class="carousel slide"
												data-bs-ride="carousel" style="padding: 42px;">
												<div class="carousel-indicators">
													...
													foreach ($img as $img) {
														...
														?>
														<button type="button"
															data-bs-target="#carouselExampleIndicators<?= $k + 1 ?>"
															data-bs-slide-to="<?= $cnt ?>" <?= ($cnt == '2') ? 'class="active" aria-current="true"' : ""; ?> class=""
															aria-label="Slide <?= $cnt ?>"></button>
													<?php } ?>
												</div>
												<div class="carousel-inner">
													...
													foreach ($img as $img) {
														...
														<div class="carousel-item <?= ($ss == 1) ? 'active' : ''; ?>">
															<img class="d-block"
																src="<?= base_url('external/uploads/'); ?><?= $img->ppt; ?>"
		...
		<div class="invt-emply-bx">
			<div class="row">
				...
				<div class="col-xl-6 col-sm-12">
					...
					<a href="<?= base_url('client/employees/' . $myid) ?>"
						class="btn btn-primary btn-rounded fs-18">Manage employees</a>
				</div>
				<div class="col-xl-6 col-sm-12">
					...
					<button type="button" class="btn btn-primary fs-18" data-bs-toggle="modal"
						data-bs-target="#inviteempmodal">
						Invite employees to register
					</button>
				</div>
				...
				<div class="card">
					...
					<div class="card-header">
						...
						<div class="see-btn batabase-table-btn" style="margin-bottom: 20px;">
							<a href="javascript:void(0);" onclick="document.getElementById('mailajob').submit();"
								class="btn btn-outline-info btn-rounded fs-18">Approve</a>
							<a href="javascript:void(0);"
								onclick="document.getElementById('appr-inp').value='0';document.getElementById('mailajob').submit();"
								class="btn btn-outline-info btn-rounded fs-18">Reject</a>
							...
							<a href="<?= base_url('client/downloadend/' . $myid . '/3') ?>"
								class="btn btn-outline-info btn-rounded fs-18">Download</a>
						</div>
							...
							<table id="example2" class="table" class="display" style="min-width: 845px">
								...
								<tbody>
									...
									foreach ($res as $res) {
										...
										?>
										<tr>
											<td><input type="checkbox" value="<?= $res->eid ?>" name="eid[]"
											...
											<td><a href="<?= base_url('client/addemployee/' . $myid . '/' . $res->eid) ?>"
													class="badge badge-success"><?= ($res->status == 3) ? "Hr Approval Pending" : (($res->status == 2) ? "Under process" : "") ?></a></td>
											<td>
												<?php if (!empty($res->pimage)): ?>
													<a target="_blank"
														href="<?= base_url('external/uploads/' . $res->pimage) ?>">View</a>
												<?php else: ?>
													N/A
												<?php endif; ?>
											</td>
											<td>
												<?php echo getDMYDate($res->up_time, false); ?>
											</td>
										</tr>
									...
									foreach ($tot as $rr) {
										...
										?>
										<tr>
											<td><input type="checkbox" value="<?= $rr->did ?>" name="did[]"
													class="checkboxall" /></td>
											<td>
												<?= $zz->emp_id ?>
											</td>
											...
											<td>
												<?= ($rr->reltype == 'Kid1' || $rr->reltype == 'Kid2' || $rr->reltype == 'Kid3') ? "Kid /" : $rr->reltype . " /" ?><br>
												<?= $rr->gender ?>
											</td>
											<td>
												<?= $rr->mode ?>
											</td>
											<td><a href="<?= base_url('client/addemployee/' . $myid . '/' . $rr->eid) ?>"
													class="badge badge-success"><?= ($rr->status == 3) ? "Hr Approval Pending" : (($rr->status == 2) ? "Under process" : "") ?></a></td>
											<td>
												<?php if (!empty($rr->pimage)): ?>
													<a target="_blank"
														href="<?= base_url('external/uploads/' . $rr->pimage) ?>">View</a>
												<?php else: ?>
													N/A
												<?php endif; ?>
											</td>
											<td>
												<?php echo getDMYDate($rr->updated_on, false); ?>
											</td>
										</tr>
										<?php
									} ?>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="upload-emp-form" method="post" enctype="multipart/form-data">
					...
					<h6>Note:-Download Template <a href="<?php echo base_url(); ?>external/employee_format.xlsx"><i
