...
<form method="POST" action="<?= base_url('client/adddep/'); ?><?= $pid; ?>/<?= $eid; ?>" enctype="multipart/form-data">
   <div class="row">
      ...
      <div class="mb-3 col-md-6">
         ...
         <input type="date" name="dob" id="dob2" required style="background:#f8f8f8" data-age-inp="<?php echo $dobId; ?>" class="form-control calcage" value="<?= !empty($spouse->dob) ? $spouse->dob : ''; ?>" placeholder="Dob">
      </div>
      <div class="mb-3 col-md-6">
         <label class="form-label">Age</label>
         <input type="text" name="age" id="<?php echo $dobId; ?>" required style="background:#f8f8f8" class="form-control" value="<?= !empty($spouse->age) ? $spouse->age : ''; ?>" placeholder="Age">
      </div>
      ...
      <input type="hidden" name="reltype" value="Mother In Law">
   ...
   <?php if ($spouse->name) { ?>
      ...
      <?php if ($spouse->status != 2) : ?>
         ...
         <button class="btn btn-primary" type="submit">UPDATE MOTHER IN LAW</button>
         <a href="javascript:;" onclick="deleterec(<?php echo $did; ?>)" class="btn btn-primary" type="submit">Delete</a>
      <?php else : ?>
         <button disabled class="btn btn-primary" type="button"><?php echo getEmployeeStatus($spouse->did, 'Mother In Law')['statustext']; ?></button>
      <?php endif; ?>
   <?php } else { ?>
      <input type="hidden" style="background:#f8f8f8" name="mode" value="New Addition">
      <button class="btn btn-primary" type="submit">ADD MOTHER IN LAW</button>
   <?php } ?>
